...
function PaginationCustom(props: PaginationCustomProps) {
  ...
  const [totalCountPage] = useState<Array<number>>(
    Array.apply(null, Array(totalPage)).map(function ( _x:any, i: number) {
      return i + 1;
    })
  );
      ...
      <div className="flex flex-row gap-2 text-[#fff]">
        {getDifference(totalCountPage.length, page) <= 3 ? (
          <div className="flex flex-row items-center gap-4 w-11/12 justify-center">
            <span onClick={() => setPage(1)} className="">
              1
            </span>
            <span>...</span>
            <span
              onClick={() =>
                setPage(
                  totalCountPage.slice(
                    Math.max((totalCountPage.length as number) - 4, 0)
                  )[0]
                )
              }
              className={` ${
                totalCountPage.slice(
                  ...
                  ? "rounded-full bg-blue-900 px-4 py-2"
                  : ""
              } ${
            ...
            {totalCountPage
              .slice(Math.max((totalCountPage.length as number) - 3, 0))
              .map((item: number) => (
                <div
                  key={item}
          ...
          <div className="flex flex-row items-center gap-4 w-11/12 justify-center">
            ...
            <div
              className={`${page ? "rounded-full bg-blue-900 px-4 py-2" : ""}`}
            >
              {page}
            </div>
            <div onClick={() => setPage(page + 1)}>{page + 1}</div>
            <span>...</span>
            <span className="" onClick={() => setPage(totalCountPage.length)}>
              {totalCountPage.length}
            </span>
          </div>
        )}
      </div>
